...
QCovid_cohort <- QCovid_cohort %>% mutate(any_comorb = select(., Q_BMI:Q_DIAG_VTE) %>% rowSums(na.rm = TRUE))
...
qcovid_addvars <- readRDS(paste0(Location, "EAVE/GPanalysis/data/QCOVID_BMIupdate.rds") )
